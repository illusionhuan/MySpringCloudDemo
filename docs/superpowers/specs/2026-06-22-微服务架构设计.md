...
/api/comments/**       → content-service    (需 Token)
/api/notifications/**  → content-service    (需 Token)
```
...
**职责**：帖子管理、评论管理、通知管理（合并原 comment-service 和 notification-service）

**数据库**：`appdb`（PostgreSQL，表名 `contents`、`comments`、`notifications`）
...
public class Comment {
    private Long id;
    private Long contentId;    // 关联帖子 ID
...
**内部调用**：创建评论时通过 `ContentService.findById()` 验证帖子存在

**Kafka 事件**：评论模块发送 `comment-events` Topic → 通知模块消费并生成通知
...
| 服务 | 数据库名 | 端口 | 表名 |
|------|----------|------|------|
| user-service | appdb | 5432 | users |
| content-service | appdb | 5432 | contents, comments, notifications |

所有服务在同一 PostgreSQL 实例的 `appdb` 数据库中，通过不同的表名隔离。
...
|--------|-----|
| 地址 | localhost:9092 |
| comment-events Topic | content-service 评论模块生产，通知模块消费 |
...
| 阶段 | 内容 | 学习重点 |
|------|------|----------|
| 阶段 1 | 搭建多模块骨架 + common-module | Maven 多模块、Nacos 注册 |
| 阶段 2 | 实现 user-service（注册登录 + CRUD） | JWT 签发、MyBatis-Plus |
| 阶段 3 | 实现 content-service + OpenFeign 调用 | 服务间同步调用、Feign |
| 阶段 4 | 实现 Gateway Service + JWT 鉴权 | API 网关、路由、JWT 认证 |
| 阶段 5 | content-service 集成评论 + Kafka 事件 | 异步消息、事件驱动 |
| 阶段 6 | 前端 Vue 3 联调（7 个页面） | 前后端分离、JWT 传递 |
